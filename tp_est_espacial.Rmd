...
grilla_bbox <-  st_make_grid(elevaciones, cellsize = c(700, 700)) %>% 
...
```{r, eval=FALSE, echo=FALSE}
elevaciones_grilla <- st_intersection(grilla_bbox, st_as_sf(elevaciones_boundaries)) %>%
  st_sf()
```


Como la intersección entre la grilla de puntos y los límites de la ciudad toma tiempo
```{r eval=FALSE, echo=FALSE}
#transformamos los datos para poder almacenarlos como shapefile
grilla_muestra <- st_zm(elevaciones_grilla, drop=T, what='ZM')

#guardamos el dataframe elevaciones_NY_muestra
st_write(grilla_muestra,
         "elevaciones_grilla.shp")
```
